    ...
    cpuRef.once("value", snapshot => {
      ...
      if (cpus) {
        ...
        const filteredCpus = Object.values(cpus).filter(cpu => {
          ...
          const isGraphicsMatch = value === "0" || value === "1" ? cpu["내장그래픽 여부"] === "O" : cpu["내장그래픽 여부"] === "X";
    ...
    vgaRef.once("value", snapshot => {
      ...
      if (vgas) {
        ...
        if (filteredVgas.length > 0) {
          ...
          recommendationResult.innerHTML = '<p>적합한 그래픽 카드를 찾을 수 없습니다.</p>';
        }
      } else {
        recommendationResult.innerHTML = '<p>그래픽 카드 데이터를 불러올 수 없습니다.</p>';
      }
    });
  }
