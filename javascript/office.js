    ...
    cpuRef.once("value", (snapshot) => {
      ...
      if (cpus) {
        ...
        const filteredCpus = Object.values(cpus).filter(cpu => {
          ...
          return cpuPrice >= cpuminPrice && cpuPrice <= cpumaxPrice && isBrandMatch;
        });
        ...
        if (filteredCpus.length > 0) {
          ...
          randomCpus.forEach(cpu => {
            socketMemorySpecs.push({ socket: cpu["소켓"], memory: cpu["메모리 규격"] });
          });
...
coolerElements.forEach((element, index) => {
  ...
  if (cooler) {
    ...
    if (coolerPriceElement) {
      ...
      console.log(cooler["이름"], coolerPrice);
    }
  } else {
    if (coolerElement) {
